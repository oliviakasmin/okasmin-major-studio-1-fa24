...
const analyzeDataWithRita = (lines) => {
	const frequencyMap = new Map();

	lines.forEach((line) => {
		// change our input to a Rita string
		const rs = new RiString(line);
		// console.log({ rs });
		...
		// filter out anything that is not a noun, verb, or adjective
		words.filter((word, i) => {
			if (
				["nn", "nns", "nnp", "nnps"].includes(pos[i]) ||
				pos[i] === "vb" ||
				["jj", "jjr", "jjs"].includes(pos[i])
			) {
				return word;
			}
		});
